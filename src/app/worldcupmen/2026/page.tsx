...
function WorldCup2026() {
  ...
  useEffect(() => {
    async function fetchData () {
      ...
      const baseUrl = /localhost/.test(window.location.href) 
        ? "http://localhost:3500" 
        : "https://euro21-api.vercel.app"
      const response = await fetch(`${baseUrl}/worldcupmen?year=2026&lang=${mylang}`)
      const data = await response.json()
      setMatches(await data)
      setLang(await mylang)
      document.title = `${locales("WorldCup", mylang)} 2022`
    }
    fetchData()
  },[])
  return (
